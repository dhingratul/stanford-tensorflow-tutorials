...
x = tf.random_normal([300], mean=5, stddev=0.1)
y = tf.random_normal([300], mean=5, stddev=0.1)
average = tf.reduce_mean(x - y)
def mse(): return tf.reduce_mean(tf.square(x - y))
def f2(): return tf.reduce_sum(tf.abs(x - y))
out = tf.cond(average < 0.0, mse, f2)
